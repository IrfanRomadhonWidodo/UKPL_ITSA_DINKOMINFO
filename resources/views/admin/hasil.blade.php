                ...
                <div class="flex items-center space-x-4">
                    <div class="flex items-center space-x-2">
                        <span class="text-sm font-medium text-gray-700">Total: <span
                                class="text-orange-400">{{ $hasil->total() }}</span> hasil</span>
                    </div>
                </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gradient-to-r from-[#EDBC19] to-[#8F181A]">
                    <tr>
                        <th scope="col"
                            class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Foto
                        </th>
                        <th scope="col"
                            class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Aplikasi
                        </th>
                        <th scope="col"
                            class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Pengaju
                        </th>
                        <th scope="col"
                            class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Tautan
                        </th>
                        <th scope="col"
                            class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Tanggal
                        </th>
                        <th scope="col"
                    ...
                    @forelse ($hasil as $item)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <img src="{{ $item->image ? asset('storage/' . $item->image) : asset('images/default-user.png') }}"
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center space-x-3">
                                    <button id="btn-view-{{ $item->id }}"
                                        onclick="openModal('viewHasilModal{{ $item->id }}')"
                                    ...
                                    <button id="btn-edit-{{ $item->id }}"
                                        onclick="openModal('editHasilModal{{ $item->id }}')"
                                    ...
                                    <form id="delete-form-{{ $item->id }}"
                                        action="{{ route('admin.hasil.destroy', $item->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" onclick="deleteHasil({{ $item->id }})"
                        ...
                        <!-- Modal View Hasil -->
                        <div id="viewHasilModal{{ $item->id }}"
                            class="fixed inset-0 flex items-center justify-center z-50 hidden">
                            <div class="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
                                <!-- Modal Header -->
                                <div
                                    class="sticky top-0 bg-gradient-to-r from-[#EDBC19] to-[#8F181A] px-6 py-4 rounded-t-lg">
                                    ...
                                    <div class="space-y-6">
                                            ...
                                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                <div class="space-y-2">
                                                    <div class="flex flex-col">
                                                        ...
                                                        <span
                                                            class="text-gray-900 font-semibold">{{ $item->formulir->nama_aplikasi }}</span>
                                                    </div>
                                                </div>
                                                <div class="space-y-2">
                                                    <div class="flex flex-col">
                                                        ...
                                                        <span
                                                            class="text-gray-900">{{ $item->formulir->domain_aplikasi }}</span>
                                                    </div>
                                                </div>
                                                <div class="space-y-2">
                                                    <div class="flex flex-col">
                                                ...
                                                <div class="space-y-2">
                                                    <div class="flex flex-col">
                                                        ...
                                                        <span
                                                            class="text-gray-900">{{ $item->formulir->user->email }}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <div class="space-y-4">
                                                ...
                                                <div class="space-y-2">
                                                    ...
                                                    <div class="bg-white p-4 rounded-lg border">
                                                        <p class="text-gray-900 whitespace-pre-line">{{ $item->deskripsi }}
                                                        </p>
                                                    </div>
                                                </div>
                                                @if($item->tautan)
                                        ...
                                        <div class="bg-gray-50 rounded-lg p-6">
                                            ...
                                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                <div class="space-y-2">
                                                    <div class="flex flex-col">
                                                        <span class="font-medium text-gray-600 text-sm">Tanggal
                                                            Dibuat</span>
                                                        <span
                                                            class="text-gray-900">{{ $item->created_at->format('d M Y, H:i') }}</span>
                                                    </div>
                                                </div>
                                                <div class="space-y-2">
                                                    <div class="flex flex-col">
                                                        <span class="font-medium text-gray-600 text-sm">Terakhir
                                                            Diperbarui</span>
                                                        <span
                                                            class="text-gray-900">{{ $item->updated_at->format('d M Y, H:i') }}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
                                <div class="sticky top-0 bg-white px-6 py-4 border-b border-gray-200 rounded-t-lg">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <button onclick="closeModal('editHasilModal{{ $item->id }}')"
                                ...
                                <form action="{{ route('admin.hasil.update', $item->id) }}" method="POST"
                                    ...
                                    <div class="px-6 py-4">
                                        <div class="space-y-4">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-sm text-gray-500 mt-1">Kosongkan jika tidak ingin mengubah
                                                    gambar</p>
                                            </div>
                                            <div>
                                    ...
                                    <div
                                        class="sticky bottom-0 bg-white px-6 py-4 border-t border-gray-200 flex space-x-3 rounded-b-lg">
                                        <button type="submit"
                                            class="flex-1 px-4 py-2 bg-gradient-to-r from-[#EDBC19] to-[#8F181A] text-white rounded-md hover:opacity-90 transition">
                                            Simpan Perubahan
                                        </button>
                                        <button type="button" onclick="closeModal('editHasilModal{{ $item->id }}')"
        ...
        <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <form action="{{ route('admin.hasil.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="sticky bottom-0 bg-white px-6 py-4 border-t border-gray-200 flex space-x-3 rounded-b-lg">
                    ...
                    <button type="button" onclick="closeModal('createHasilModal')"
                        class="flex-1 px-4 py-2 bg-gray-500 text-white rounded-md hover:bg-gray-600">
                        Batal
                    </button>
                </div>
