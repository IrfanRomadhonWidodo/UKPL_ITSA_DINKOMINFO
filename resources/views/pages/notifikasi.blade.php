                    ...
                    @forelse($notifikasi as $item)
                        <div class="relative group">
                            <div class="flex items-start gap-3">
                                <input type="checkbox"
                                    class="notification-checkbox w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 mt-4"
                                    data-id="{{ $item->id }}" onchange="updateDeleteButton()">

                                <a href="{{ route('notifikasi.show', $item->id) }}" class="flex-1 block p-4 rounded-xl border transition-all duration-200
                                    ...
                                    <div class="flex justify-between items-start">
                                        <div>
                                            <div class="flex items-center gap-2 mb-1">
                                                ...
                                                @if(!$item->is_read)
                                                    <span
                                                        class="text-xs bg-blue-100 text-blue-800 font-medium px-2 py-0.5 rounded-full">
                                                        Baru
                                                    </span>
                                                @endif
                                            </div>
    ...
    @push('scripts')
        ...
        <script>
            function markAllAsRead() {
                axios.post('/notifikasi/mark-all-read')
                    .then(response => {
            ...
            function deleteSelectedNotifications() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        axios.post('/notifikasi/delete-multiple', { ids: selectedIds })
                            .then(response => {
                                if (response.data.success) {
                                    Swal.fire({
                                        icon: 'success',
